...
build_singbox_config() {
  local direct_domains_json direct_cidrs_json proxy_domains_json proxy_cidrs_json route_excludes_json proxy_ip proxy_ip_cidrs_json proxy_domain_json ssh_ip route_excludes
  build_proxy_outbound_json
  ...
  proxy_ip="$(resolve_first_ip "$PROXY_ADDR" || true)"
  proxy_ip_cidrs_json="[]"
  if [ -n "$proxy_ip" ] && is_ip_literal "$proxy_ip"; then
    route_excludes="$route_excludes, $(cidr_for_ip "$proxy_ip")"
    proxy_ip_cidrs_json="$(csv_to_json_array "$(cidr_for_ip "$proxy_ip")")"
  fi
...
build_hybrid_singbox_config() {
  local direct_domains_json direct_cidrs_json proxy_domains_json proxy_cidrs_json route_excludes_json ssh_ip route_excludes endpoint_host endpoint_ip endpoint_ip_cidrs_json endpoint_domain_json
  ...
  if [ -n "$endpoint_host" ]; then
    if is_ip_literal "$(strip_brackets "$endpoint_host")"; then
      endpoint_ip="$(strip_brackets "$endpoint_host")"
    else
    ...
    if [ -n "${endpoint_ip:-}" ] && is_ip_literal "$endpoint_ip"; then
      route_excludes="$route_excludes, $(cidr_for_ip "$endpoint_ip")"
      endpoint_ip_cidrs_json="$(csv_to_json_array "$(cidr_for_ip "$endpoint_ip")")"
    fi
  fi
